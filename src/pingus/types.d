...
struct Var {
	...
	this(HostFunc f, string funcName, string callConv, bool varArg, bool noReturn) {
		FunctionEntry fe = 
				FunctionEntry(xxhash32(cast(ubyte[])funcName), xxhash32(cast(ubyte[])callConv), f, varArg, noReturn);
		valR[0] = createHeapEntry(reinterpretAsArray!ubyte(fe));
	...
	this(HostDeleg d, string funcName, string callConv, bool varArg, bool noReturn) {
		FunctionEntry fe = 
				FunctionEntry(xxhash32(cast(ubyte[])funcName), xxhash32(cast(ubyte[])callConv), d, varArg, noReturn);
		valR[0] = createHeapEntry(reinterpretAsArray!ubyte(fe));
		typeID = VarTypeID.deleg;
		//metatableRef = xxhash32(cast(ubyte[])T.stringof);
	}
	this(void* stackEntry) {
		this = *(cast(Var*)stackEntry);
	}
	this(T)(T val) {
...
enum FunctionEntryFlags : uint {
	...
	memberFunc		=	1 << 3,		///Is a member of a metatable
	ext				=	1 << 4,		///External function (paired from host)
	exp				=	1 << 5,		///Export function (visible to host)
	property		=	1 << 6,		///Property function
}

alias HostFunc = ulong[] function(ref ulong[16], ulong[]);
alias HostDeleg = ulong[] function(Var, ref ulong[16], ulong[]);

struct FunctionEntry {
	uint			pkgHash;		///XXHash32 of the package name
	...
	uint			nOfStackArgs;	///Number of stack entries for host functions
	union {
		HostFunc	hostFunc;
		HostDeleg	hostDeleg;
	}
	...
	bool opEquals(const FunctionEntry other) const @nogc @safe pure nothrow {
		return this.nameHash == other.nameHash && this.callConvHash == other.callConvHash && this.pkgHash == other.pkgHash;
	}
...
struct VMInstruction {
	...
	public bool stack_getType() @nogc @safe pure nothrow const {
	...
	public bool hasPkgHash() @nogc @safe pure nothrow const {
		return (bytes[1] & 0x10) != 0;
	}
...
public struct PingusHeader {
	...
	uint		nameHash;	///Hash of the name of the package
...
public struct PingusBinFile {
	...
	FunctionEntry[]	functions;	///All non-metatable functions of the file.
